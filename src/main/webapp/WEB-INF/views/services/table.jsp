        ...
        <div class="dashboard-page-one">
           <jsp:include page="../wayupartyMasterSideNav.jsp" />
            ...
            <div class="card height-auto">
             <div id="tableDetailsLoadingDiv"></div>
                   <div class="card-body">
                        <div class="heading-layout1">
                    	 ...
                    	 <form class="new-added-form" id="add_table_service_form" name="add_table_service_form" >
                    	 	<div class="row mt-2">	
                    	 		
                    	 		<div class="col-xl-3 col-lg-6 col-12 form-group">
                                    <label>Table Pax<span class="text-danger">&nbsp;*</span></label>
                                    <select class="select2" name="tableType" id="tableType" data-validation="required"  field-name="Table Pax">
                                    </select>
                                 </div>
                                 
                                 <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    <label>Table Name<span class="text-danger">&nbsp;*</span></label>
                                    <input type="text" class="form-control"  name="tableName" id="tableName" data-validation="required validate_Space validate_AlphaNumber validate_length length1-100"  field-name="Table Name">
                                </div>
                    	 ...
                    	 <div class="row">	
						   <div class="col-12 ui-btn-wrap">
						               ...
						               <li><button type="button" class="btn-fill-lg font-normal text-light btn-gradient-yellow" onclick="javascript:window.location.href = '${Wayuparty_appUrl}/bottle?vendorUUID=${vendorUUID}'"><i class="fas fa-undo mg-l-10"></i>&nbsp;Reset</button></li>
...
<br/>
   ...
   <script type="text/javascript">
		window.onload = function () {
			getTableTypes();	
			var tableUUID = '${tableUUID}';
			if(tableUUID != null && tableUUID != '' && tableUUID != 'undefined'){
				getTableDetails(tableUUID);
    ...
    <script>
	   function getTableDetails(tableUUID){
	    	   var appUrl ='${appUrl}';
	    	   $("#tableDetailsLoadingDiv").attr('style','position:absolute; width:100%; height:180%; background-color:rgba(255,255,255,0.8); top:50px; left:0px; z-index:100;background-image:url("/resources/img/preloader.gif"); background-position:center; background-repeat:no-repeat; background-size:75px;');
	    	   $.ajax({
	    			  type: "GET",
	    			    url: appUrl+"/getVendorMasterServiceDetails?${_csrf.parameterName}=${_csrf.token}&serviceUUID="+tableUUID,
	    			     success :function(response) {
	    			    	    
	    			    	 $("select[name=tableType]").val(response.object.categoryId); 
	    			    	 document.getElementById('select2-tableType-container').innerHTML = response.object.category;
...
<script type="text/javascript">
    ...
    function getTableTypes(){
    	$("#tableDetailsLoadingDiv").attr('style','position:absolute; width:100%; height:100%; background-color:rgba(255,255,255,0.8); top:0px; left:0px; z-index:100;background-image:url("/resources/img/preloader.gif"); background-position:center; background-repeat:no-repeat; background-size:75px;');
    	 var appUrl ='${appUrl}';
		 ...
		 $.ajax({
		        ...
		        success: function(response) {
		        	 var output = "";
		        	output = output + "<option value=''>Select Table Pax</option>";
		        	...
		        	$("#tableType").empty();
			        $("#tableType").append(output); 
			        $('#tableType').trigger("chosen:updated");
			        $("#tableDetailsLoadingDiv").removeAttr("style");
		        },
		    });
    }
    ...
    function validateVendorService(){
    	 ...
    	 if(startDate != null  && startDate != '' && startDate != 'undefined' && 
    		 ...
    		 formData.append("masterServiceUUID", tableUUID);
 ...
 function saveTableService(){
	   if ($('#add_table_service_form').validate(false, validationSettings)){
           ...
           var serviceUUID = "${serviceUUID}";
           var tableUUID = "${tableUUID}";
           var formData = getWayupayFormData("add_table_service_form");
           formData.append("vendorUUID",vendorUUID);
           formData.append("serviceUUID",serviceUUID);
           formData.append("tableUUID",tableUUID);
             $.ajax({
				 type : "POST",
				 data: formData,
	    	     processData: false,
	    	     contentType:false,
	        	    	 url: appUrl+"/saveTableService?${_csrf.parameterName}=${_csrf.token}", 
				        success : function(result) {
						 	     ...
						 	     $('#invalidMsgDiv').delay(5000).fadeOut('slow'); 
				        		   ...
				        		   $("#vendorDetailsLoadingDiv").removeAttr("style");
				    		        location.href = "/errorPage";
							         ...
							         $("#successMsg").html("Well done! You successfully added new table service.");
 ...
 $('body').on('blur', '#add_table_service_form', function() {
		$("#add_table_service_form").showHelpOnFocus();
		$("#add_table_service_form").validateOnBlur(false, validationSettings);  
   });
 </script>
  ...
  <script>
   ...
   $(function () {
       $("#uploadImageBtn").change(function () {
           ...
           fileUpload("uploadImageBtn", "vendor_services_images", "add_table_service_form",vendorId,0);
           $(".clearUpload").show();
       });
   });
 </script>
